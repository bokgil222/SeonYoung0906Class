...
public class SmartPhoneUsedList {
	...
	public void insertContact(int select) {
		...
		if(checkPhoneNumber(phoneNumber)) {
			System.out.println("입력하신 전화번호가 이미 존재합니다.");
		...
		if (select == 1) {
			...
			addContact(new CompanyContact(name, phoneNumber, email, address, 
			...
			addContact(new CompanyContact(name, phoneNumber, email, address, 
					birthday, group, companyName, product, job));
		}
	}
	
	// 2. 연락처 이름 입력 시 공백에 대한 예외처리와 영문자와 한글만 허용하는 예외 처리를 해봅시다.
		// 이름 입렵 시 공백을 허용 하지 않고, 영문자와 한글만 가능하도록 처리
	private String getName() {
		
		String result = null;
		
		// 사용자에 문자열 받고-> 예외 발생 여부 확인 -> 예외 발생 -> 예외 처리
		while (true) {
			// 데이터 받기
			...
			if (result.length() > 0) {
				...
				// 문자열에 영문자와 한글만 포함되어 있는지 체크하는 변수
				boolean check = false;
	...
	private String getString() {
		...
		while(true) {
			...
			if(result.length() == 0) {
				System.out.println("공백 입력은 허용하지 않습니다. 다시 입력해주세요.");
			} else {
				break;
			}
		}
