...
//		}
    ...
    // 2. 연락처 이름 입력 시 공백에 대한 예외 처리와 영문자와 한글만 허용하는 예외 처리를 해봅시다.
	// 이름 입력 시 공백 허용 하지 않고, 영문자와 한글만 가능하도록 처리
	private String getName() {
	    ...
	    while(true) {
	    	...
	    	if(result.length() > 0) {
	    		...
	    		// 문자열에 영문자와 한들만 포함하고 있는지 체크하는 변수
	    		boolean check = false;
	...
	// 3. 전화번호 형식에 맞지 않을 때 예외 처리를 하고 중복될 때 예외 상황이 발생하도록 하고 예외 처리를 합시다.
