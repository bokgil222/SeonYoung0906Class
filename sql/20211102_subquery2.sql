...
-- 2073.21428571485714857148571485714
select ename, sal
from emp
...
-- 각 고객의 평균 주문 금액보다 큰 금액의 주문내역에 대해서 주문번호, 고객번호, 금액을 보이시오.
-- 각 고객의 평균 주문 금액
-- 조건: 각 고객의 평균 주문 금액보다 큰 금액의 주문내역
-- 주문내역에 대해서 주문번호, 고객번호, 금액을 보이시오.
select avg(saleprice) from orders where custid = 4;

select custid, orderid, saleprice
from orders O
where saleprice > (
     select avg(saleprice) from orders OA where custid = O.custid
     )
;
...
-- 급여 중 가장 작은 값보다 많은 급여를 받는 사원의 이름
select ename, sal
from emp
...
where EXISTS (
      select *
      from customer c
      where address like '%대한민국%' and custid = 3
      )
;
