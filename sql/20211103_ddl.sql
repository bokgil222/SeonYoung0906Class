...
-- 사원 테이블과 유사한 구조의
-- 사원번호, 사원이름, 급여 3개의 칼럼으로 구성되 EMP01 테이블을 생성해 봅시다.
desc emp;
...
desc emp01;
-- EMP01 테이블에 문자 타입의 직급(JOB) 컬럼을 추가해봅시다.
alter table emp01
add (job varchar2(9))
;
select * from emp01;
...
-- 직급(JOB) 칼럼을 최대 30글자까지 저장할 수 있게 변경해 보자.
alter table emp01
...
-- EX01 테이블의 직급 칼럼을 삭제해 보자.
alter table emp01 
drop column job;

-- 테이블의 모든 튜플을 삭제: truncate table
-- 삭제와 동시에 commit: 바로 물리적인 반영(저장)
...
-- EMP02 테이블을 생성하되
-- EMPNO와 ENAME 컬럼에 NOT NULL 제약 조건을 설정해 봅시다.
-- 테이블 삭제
drop table emp02;
create table emp02 (
