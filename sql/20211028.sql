...
-- null 값을 치환해주는 nvl함수
-- nvl(컬럼명, 치환할 데이터) => 컬럼의 값중 null 값은 치환 값으로 출력
...
-- 사원들이 소속된 부서 번호 출력 (distinct: 중복된 행을 한 번씩만 출력)
select distinct deptno 
from emp
;

-- 우리 서점에 입점한 출판사 리스트(중복 빼고)
...
select *
from emp
where deptno=10 and job='MANAGER'  --' '안의 문자열 데이터는 대소문자 구별
;
...
--  이름의 두 번째 글자가 A 인 사원을 검색
select *
from emp
where ename like '_A%'
;

-- 이름 중 A를 포함하지 않는 사원을 검색
select *
from emp
...
select *
from emp
order by hiredate, sal asc  -- 같은 입사일의 경우 급여 오름차순으로 정렬
;
